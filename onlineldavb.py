...
class OnlineLDA:
    ...
    def __init__(self,  K,len_voc, D, alpha, eta, tau0, kappa):
        """
        Arguments:
        K: Number of topics
        ...
        self.phi = n.zeros((D,K))

    def do_e_step(self, wordids, wordcts, updated_alpha):
        ...
        for d in range(0, batchD):
            ...
            for it in range(0, 100):
                ...
                if (meanchange < meanchangethresh):
                    break
            gamma[d, :] = gammad
            self.phi[d, :] = expElogthetad * n.dot(cts / phinorm, expElogbetad.T)
        ...
        self._gamma = gamma
        self.phisum = expElogthetad * n.dot(cts / phinorm, expElogbetad.T)
...
#         return((gamma, sstats))
    ...
    def update_lambda(self, wordids, wordcts, updated_alpha):
        """
        First does an E step on the mini-batch given in wordids and
        wordcts, then uses the result of that E step to update the
        variational parameter matrix lambda.
        ...
        # mini-batch. This also returns the information about phi that
        # we need to update lambda.
        (gamma, sstats) = self.do_e_step(wordids, wordcts, updated_alpha)
        # Estimate held-out likelihood for current values of lambda.
...
#             score += n.sum(cts * n.log(phinorm))
        ...
        for it in range(self._alpha.shape[0]):
            score += n.sum((self._alpha[it] - gamma)*Elogtheta)
            score += n.sum(gammaln(gamma) - gammaln(self._alpha[it]))
            score += sum(gammaln(self._alpha[it]*self._K) - gammaln(sum(gamma)))
        score = score/self._alpha.shape[0]
...
#             score += n.sum(cts * n.log(phinorm))
        ...
        # E[log p(theta | alpha) - log q(theta | gamma)]
        for it in range(self._alpha.shape[0]):
            score += n.sum((self._alpha[i] - gamma)*Elogtheta)
            score += n.sum(gammaln(gamma) - gammaln(self._alpha[i]))
            score += sum(gammaln(self._alpha[i]*self._K) - gammaln(n.sum(gamma, 1)))
        score = score/self._alpha.shape[0]
    ...
    def inference(self, wordids, wordcts, updated_alpha):
        self._alpha = updated_alpha
        batchD = len(wordids)
        doc_test = n.zeros((updated_alpha.shape[0], self._K))
        ...
        for d in range(0, batchD):
            ...
            for it in range(0, 100):
                ...
                if (meanchange < meanchangethresh):
                    break
            gamma[d, :] = gammad
            doc_test[d, :] = expElogthetad * n.dot(cts / phinorm, expElogbetad.T)
            # Contribution of document d to the expected sufficient
            # statistics for the M step.
            sstats[:, ids] += n.outer(expElogthetad.T, cts/phinorm)
        ...
        sstats = sstats * self._expElogbeta
        self.phisum = expElogthetad * n.dot(cts / phinorm, expElogbetad.T)
        doc_test = doc_test/doc_test.sum(axis=1)[:,None]

        return((gamma, sstats, doc_test))
